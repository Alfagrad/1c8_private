...
class OrderController extends BaseController
{
    ...
    public function postNewOrder(XMLHelper $XMLHelper, Request $request){
        ...
        //TODO убрать этот код - нужен один раз только
        // $contacts = ProfileContact::all();
        ...
        if(isset($this->xml_data->replay) and trim($this->xml_data->replay)){
            $records = Order::with('items', 'profileId')->where('is_replay', 1)->get();
        }  else {
            $records = Order::with('items', 'profileId')->where('is_send', 0)->get();
            // Убираем статус повтора с остальных
            Order::with('items', 'profileId')->where('is_replay', 1)->update(['is_replay' => 0]);
        }



        //$records = Order::with('items', 'profileId')->where('is_send', 0)->get();
        $firstRecord = Order::with('items', 'profileId')->where('is_send', 0)->first();
