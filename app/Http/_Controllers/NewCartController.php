...
class NewCartController extends Controller
{
    ...
    public function index(Request $request, PartnerRepository $partnerRepository, AgreementTypeRepository $agreementTypeRepository, CartOrderRepository $cartOrderRepository)
    {
        ...
        // корзины юзера
//        $user_carts = CartOrder::where('profile_id', $profile->id)->get();
//        dd(auth()->user()->cartOrders);
...
//        dd($count_all_items, $items);
        ...
        // дополняем коллекцию корзин
//        $collect->push([
//            'cart_id' => 0,
//            'name' => 'Основная',
//            'count_items' => $items->count(),
//            'count_all_items' => $carts->sum('count'),
//            'items' => $items,
//        ]);

        // если есть дополнительные корзины
//        if ($user_carts->count()) {
//            foreach ($user_carts as $addition_cart) {
//
//                // собираем 1c_id товаров в корзине
//                $cart_items = $cart->where('cart_order_id', $addition_cart->id)->pluck('item_1c_id');
//
//                // собираем товары
//                $items = Item::whereIn('id_1c', $cart_items)->get();
//
//                // количество товарных позиций
//                $count_items = $items->count();
//
//                // для количества товаров всего
//                $count_all_items = 0;
//
//                // добавляем в коллекцию items количество товара
//                foreach ($items as $item) {
//
//                    // определяем количество
//                    $count_item = $cart->where('cart_order_id', $addition_cart->id)->where('item_1c_id', $item->id_1c)->first()->count;
//
//                    // добавляем в коллекцию
//                    $item->setAttribute('count_in_cart', $count_item);
//
//                    // добавляем количество товаров всего
//                    $count_all_items += $count_item;
//                }
//
//                // дополняем коллекцию корзин
//                $collect->push([
//                    'cart_id' => $addition_cart->id,
//                    'name' => $addition_cart->name,
...
//        $data['carts'] = $collect;
        $data['carts'] = $carts;
        // *********************************************************************
        Log::info('Cart partners');
        // собираем uuid-ы партнеров юзера
//        list($partner_uuids, $delivery_addresses) = $partnerRepository->get();

//        $partner_uuids = [];
//
//        // собираем адреса доставок
//        $delivery_addresses = new Collection;
//
//        // если есть партнер юзера
//        if ($profile['partner_uuid']) {
//
//            // берем uuid и адреса партнера
//            $partner = Partner::where('uuid', $profile['partner_uuid'])
//                ->first(['uuid', 'address', 'warehouse']);
//
//            // если не пусто
//            if ($partner) {
//
...
//                }
//
//                // если есть адрес или склад
//                if ($partner->address || $partner->warehouse) {
//                    // добавляем в массив
//                    $partner_uuids[] = $profile['partner_uuid'];
//                }
//
//                // берем uuid и адреса дочерних партнеров
//                $child_partners = Partner::where('parent_uuid', $profile['partner_uuid'])
//                    ->get(['uuid', 'address', 'warehouse']);
//
//                // если есть
//                if (count($child_partners)) {
//
//                    foreach ($child_partners as $child_partner) {
//
//                        // если есть адрес
//                        if ($child_partner->address) {
//                            // дописываем в коллекцию
//                            $delivery_addresses->push([
//                                'uuid' => $child_partner->uuid,
//                                'address' => $child_partner->address,
//                            ]);
//                        }
//
//                        // если есть склад
//                        if ($child_partner->warehouse) {
//                            // дописываем в коллекцию
//                            $delivery_addresses->push([
//                                'uuid' => $child_partner->uuid,
//                                'address' => $child_partner->address,
...
//
//                        // если есть адрес или склад
//                        if ($child_partner->address || $child_partner->warehouse) {
//                            // добавляем в массив
//                            $partner_uuids[] = $child_partner->uuid;
//                        }
//
//                        // берем uuid и адреса дочерних партнеров
//                        $child_child_partners = Partner::where('parent_uuid', $child_partner->uuid)
//                            ->get(['uuid', 'address', 'warehouse']);
//
//                        // если есть
//                        if (count($child_child_partners)) {
//
//                            foreach ($child_child_partners as $child_child_partner) {
//
//                                // если есть адрес
//                                if ($child_child_partner->address) {
//                                    // дописываем в коллекцию
//                                    $delivery_addresses->push([
//                                        'uuid' => $child_child_partner->uuid,
//                                        'address' => $child_child_partner->address,
//                                    ]);
//                                }
//
//                                // если есть склад
//                                if ($child_child_partner->warehouse) {
//                                    // дописываем в коллекцию
//                                    $delivery_addresses->push([
//                                        'uuid' => $child_child_partner->uuid,
//                                        'address' => $child_child_partner->address,
...
//
//                                // если есть адрес или склад
//                                if ($child_child_partner->address || $child_child_partner->warehouse) {
//                                    // добавляем в массив
//                                    $partner_uuids[] = $child_child_partner->uuid;
//                                }
//                            }
//                        }
//                    }
...
//        }
//
//        $data['delivery_addresses'] = $delivery_addresses->sortBy('address');

//        Log::info('Cart agreements');

        // персональные соглашения
//        $personal_agreements = $agreementTypeRepository->personal($partner_uuids);

//        $personal_agreements = AgreementType::whereIn('partner', $partner_uuids)
...
//
//            foreach ($personal_agreements as $key => $agreement) {
//
...
//
//                    // добавляем в коллекцию
//                    $personal_agreements[$key]->items_str = $items_str;
//                }
//            }
//        }

//        $data['personal_agreements'] = $personal_agreements;

        // стандартные соглашения
        $common_agreements = $agreementTypeRepository->common();

//        $common_agreements = AgreementType::where([['partner', ''], ['date', '<=', date('Y-m-d')]])
//            ->where(function ($query) {
//                $query->where('date_end', '0000-00-00')->orWhere('date_end', '>=', date('Y-m-d'));
//            })
//            ->get()->sortBy('name');
...
//
//            foreach ($common_agreements as $key => $agreement) {
//
//                // берем товары соглашения
//                $agreement_items = AgreementProduct::where('agreement_uuid', $agreement->uuid)->get();
//
...
//        }
        ...
        return view('new_cart')->with($data);
    }
