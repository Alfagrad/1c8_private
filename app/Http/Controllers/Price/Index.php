...
//        $data = $this->getUserData($request);
        ...
        // собираем категории
        $categories = Category::with('subCategory')->withCount(['items', 'items as archived_items' => function ($q) {
            return $q->where('in_archive', 1);
        }])
            ->where('parent_1c_id', 0)
            ->whereNotIn('id_1c', [20070, 193])
            ->orderBy('default_sort')
            ->get(['name', 'id_1c']);

//        foreach ($isCategoryForPrice->categories() as $category => $method) {
//            $additionalCategory = new Category(['name' => $category]);
//            $categories->prepend($additionalCategory);
//        }
