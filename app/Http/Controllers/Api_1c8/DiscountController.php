...
class DiscountController extends BaseController
{
    ...
    public function discountCreateOrUpdate(XMLHelper $XMLHelper, Request $request)
    {
        ...
        if ($this->xml_data->discounts->discount && $this->xml_data->discounts->discount->count()) {
            ...
            if ($common_discount) {
                ...
                $product_uuids = DiscountProduct::where('discount_uuid', $uuid)->pluck('item_uuid')->toArray();
    ...
    public function agreementCreateOrUpdate(XMLHelper $XMLHelper, Request $request)
    {
        ...
        if ($common_discount) {
            ...
            // берем uuid товаров дисконта
            $product_uuids = DiscountProduct::where('discount_uuid', $discount_uuid)->pluck('item_uuid')->toArray();
            ...
            if ($this->xml_data->product->count()) {
                ...
                foreach ($this->xml_data->product as $value) {
                    ...
                    if ($value->products->product && $value->products->product->count()) {
                        ...
                        // берем uuid товара
                        $data['item_uuid'] = trim($product->uuid);

                        // проверяем, есть ли в товар в БД
                        $item = Item::where('uuid', $data['item_uuid'])->first(['uuid']);

                        // если нет
                        if (!$item) {
                            // добавляем в ответ
                            $answer .= "Товар ".$data['item_uuid']." отсутствует в БД. Пропущен.\n";
                            ...
                            if (!empty($data['date_end']) && strtotime($data['date_end']) < time()) {
                                ...
                                // добавляем в ответ
                                $answer .= "Дисконт для товара ".$data['item_uuid']." просрочен. Пропущен.\n";
