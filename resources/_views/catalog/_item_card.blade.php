...
if($item_card->discount_str) {

    // делим скидки
    $discounts = explode(';', $item_card->discount_str);
    ...
    foreach ($discounts as $element) {
        ...
        if (date('Y-m-d') >= $date_start && ($date_end == '0000-00-00' || date('Y-m-d') <= $date_end)) {
            ...
            if($discount_count > 1) {
                // формируем строку 2
                $discount_str_2 .= "от {$condition} шт ".number_format($discount_price * 1.2, 2, '.', '')." руб<br>";
    ...
    // удаляем тег переноса в конце строки
    $discount_str_2 = preg_replace('/<br>$/', '', $discount_str_2);
...
<div class="item-page">
                ...
                <div class="item-page_thumb-images">
                    ...
                    @if($item_card->guides->count())
                        ...
                        @foreach($item_card->guides as $guide)
                            ...
                            @if($item_card->guides->count() > 1) @php $add = " ".$loop->iteration @endphp @endif
                    ...
                    @if(!$is_service)
                        ...
                        <div class="js-item-card-opt" style="display: {{ $opt_style }};">
                            ...
                            <div class="opt-line">
                                ...
                                <div class="price-opt">
                                    <span @if(isset($line_through) && $line_through == 1) class="through" @endif>
                                        {{ price($price) }} руб
                                    </span>

                                    @if($discount_count > 1 && $discount_str_2)

                                        <div class="action-str">
                                            {!! $discount_str_2 !!}
                                        </div>

                                    @endif

                                    @if($discount_count == 1 && $discount_str)
                        ...
                        @if($discount_str || $item_card->spec_price == 1)
    ...
    {{--                                 @if($item_card->mini_text != '')
                                        ...
                                        <div class="sticker" title="{{ $item_card->mini_text }}">
                                            %
                                        </div>
...
{{--                 @if(isset($spare_items) && $spare_items->count())
            ...
            <div class="item-page_down-block js-down-block block-about active">
                ...
                @include('catalog.snippets.item_card_about_item', [
                        'opt_style' => $opt_style,
            ...
            @if($item_card->schemes && $schemes->count())
                ...
                <div class="item-page_down-block js-down-block block-spare">
                    ...
                    @include('catalog.snippets.item_card_spares', [
                            'opt_style' => $opt_style,
                            'purcent_style' => $purcent_style,
                            'mr_style' => $mr_style,
                        ])
...
{{--             @if($analogs->count())
            ...
            @if($buy_with_cat->count() || $buy_with->count())
                ...
                <div class="item-page_down-block js-down-block block-buy-with">
                    ...
                    @include('catalog.snippets.item_card_buy_with', [
                            'opt_style' => $opt_style,
                            'purcent_style' => $purcent_style,
                            'mr_style' => $mr_style,
                        ])
            ...
            @if($services->count())
                ...
                <div class="item-page_down-block js-down-block block-services">
                    ...
                    @include('catalog.snippets.item_card_services', [
                            'opt_style' => $opt_style,
                            'purcent_style' => $purcent_style,
                            'mr_style' => $mr_style,
                        ])
